...
const pains = [
  "Опыт и результаты есть — а продаж нет.",
  "Клиенты приходят нестабильно, доход зависит от запусков, ретроградного Меркурия или предсказания нумеролога",
  "Блог требует всё больше времени, а заявок всё меньше",
  "Хочется работать с сильной платежеспособной аудиторией — но непонятно, где её искать",
  "Есть ощущение, что вы уже переросли текущие продукты, но куда двигаться дальше — неясно",
  "Пробовали разные форматы, но системы так и нет",
];

const agenda = [
  {
    title: "Почему старая модель роста перестала работать",
    text: "Что изменилось в поведении клиентов и на рынке — и почему то, что работало три года назад, сейчас даёт всё меньше результата.",
  },
  {
  ...
  },
  {
    ...
    text: "Не копировать чужие схемы, а выстроить модель под свой опыт, аудиторию и цель по доходу.",
  },
  {
    title: "Как сформировать цену и не бояться, что не купят",
    text: "Почему высокий чек — это не про наглость, а про архитектуру продукта. И как объяснить ценность так, чтобы клиент сам попросил выставить счёт.",
  },
  {
    title: "Как перейти в более высокий сегмент клиентов",
    text: "Даже если сейчас вы продаёте недорогие продукты — и даже если блог маленький.",
  },
];
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      gsap.fromTo(
        ...
        { y: 0, opacity: 1, filter: "blur(0px)", duration: 0.62, stagger: 0.045, ease: "power3.out" },
      );
      gsap.fromTo(
        ".case-visual",
    ...
    <main ref={rootRef} className="site-shell">
      ...
      <section className="section host-section">
        ...
        <div className="host-copy reveal">
          ...
          <div className="host-facts">
            <span>С 2009 года в бизнес-обучении, с 2020 — в онлайн-образовании</span>
      ...
      <section className="final-cta">
        <div className="support-copy reveal">
          ...
          <p>
            Выберите удобный канал: Telegram, Max или ВКонтакте. Ссылки можно будет подставить сразу,
            как пришлете.
          </p>
